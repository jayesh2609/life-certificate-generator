...
import pytesseract
import traceback
import sys # Import the sys module for forceful printing
...
# (The data extraction and PDF generation functions below have no changes)
...
def upload_file():
    # **FIX IS HERE**: Added forceful printing for debugging
    print("--- UPLOAD ROUTE ENTERED ---", file=sys.stderr)
    sys.stderr.flush()
    try:
        ...
        if file:
            ...
            print("--- IMAGE SAVED, STARTING OCR ---", file=sys.stderr)
            sys.stderr.flush()
            
            extracted_text = pytesseract.image_to_string(image_path)
            
            print("--- OCR FINISHED, PARSING TEXT ---", file=sys.stderr)
            sys.stderr.flush()
